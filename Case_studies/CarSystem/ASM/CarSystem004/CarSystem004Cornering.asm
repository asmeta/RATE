...
module CarSystem004Cornering
import ../StandardLibrary
import CarSystem004LowBeam
export *
...
definitions:
	...
	//ELS-24
		par
			if ($leftRight = RIGHT) then
				corneringLightRight := setOverVoltageValueLight(100)
			endif
			if ($leftRight = LEFT) then
				corneringLightLeft := setOverVoltageValueLight(100)
			endif
		endpar
	...
	macro rule r_CorneringLights =
		//ELS-45 turn on cornering lights if no subvoltage
	...
	macro rule r_ReverseLight = 
		if (reverseGear) then
			reverseLight := setOverVoltageValueLight(100)
		else
			reverseLight := 0
		endif

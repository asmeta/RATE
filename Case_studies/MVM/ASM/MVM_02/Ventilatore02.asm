...
/*
	...
	No inspiratory/expiratory pause is present.
	This refinement level introduces the apnea and, thus, the passage from PSV to PCV)
*/
			...
			//go to state MAIN_REGION_VENTILATIONOFF immediately
				...
				if state=MAIN_REGION_PCV_R1_EXPIRATION and (stopVentilationRequested or stopVentilation) then 
					...
					if state=MAIN_REGION_PSV_R1_EXPIRATION and (stopVentilationRequested or stopVentilation)  then 
						...
						par
							if apneaAlarm then
								if turnOffApneaAlarm = true and (state=MAIN_REGION_PCV_R1_INSPIRATION or state=MAIN_REGION_PCV_R1_EXPIRATION) then 
									r_apneaAlarmOFF[] 
								endif
							endif

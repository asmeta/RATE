...
/* 	rule r_PSVexpIValve =
	...
	if turnOFFbtn then r_turnOff[]
		...
		if state = MAIN_REGION_PCV_R1_INSPIRATORYPAUSE then
			if (maxInPausePassed or not inPause) then r_PCVexp[]
			endif
		endif
		...
		if state = MAIN_REGION_PCV_R1_EXPIRATION	then
			if stopVentilation then r_ventOffPCV[]
				else if expirationDurationPassed then 
		...
		if state = MAIN_REGION_PCV_R1_EXPIRATORYPAUSE then
			if (maxExpPausePassed or not expPause) then r_PCVinsp[]
			endif
		endif
		...
		if state = MAIN_REGION_PSV_R1_INSPIRATORYPAUSE then
			if (maxInPausePassed or not inPause) then r_PSVexp[]
			endif
		endif
		...
		if state = MAIN_REGION_PSV_R1_EXPIRATORYPAUSE then
			if (maxExpPausePassed or not expPause) then r_PSVinsp[]
			endif
		endif
...
default init s0:
//initializing controlled functions

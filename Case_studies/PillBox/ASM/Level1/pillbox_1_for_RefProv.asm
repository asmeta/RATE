...
signature:
	...
	derived timeDiffOver600: Boolean
...
definitions:
	...
	//*************************************************
	// DERIVED FUNCTIONS AND STATIC FUNCTIONS
	//*************************************************
	function takeThePill = (time_consumption<=systemTime)
	function timeDiffOver600 = (systemTime-drawerTimer>tenMinutes)
	...
	main rule r_Main =
		if (not(requestSatisfied) and systemTime < 24) then
			par
				...
				if redLed = OFF then if (takeThePill and not requestSatisfied) then r_pillToBeTaken[] endif endif

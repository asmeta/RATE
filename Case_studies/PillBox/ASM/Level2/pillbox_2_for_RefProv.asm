...
signature:
	...
	enum domain OutMessages = {
		...
		TAKE_ASPIRINE_IN_10_MIN | TAKE_TYLENOL_IN_10_MIN | TAKE_MOMENT_IN_10_MIN 
	}
	...
	derived takeThePill: Drawer -> Boolean
	derived timeDiffOver600: Drawer -> Boolean
...
definitions:
	...
	// DERIVED FUNCTIONS
	//*************************************************
	function takeThePill($drawer in Drawer) = (time_consumption($drawer)<=systemTime)
	function timeDiffOver600($drawer in Drawer) = (systemTime-drawerTimer($drawer)>1)
	...
	rule r_takeInTimeout($drawer in Drawer) =
		par
			if redLed($drawer) != BLINKING and outMess($drawer) != TAKE_ASPIRINE_IN_10_MIN and outMess($drawer) != TAKE_TYLENOL_IN_10_MIN and 
				outMess($drawer) != TAKE_MOMENT_IN_10_MIN then drawerTimer($drawer) := systemTime endif
			redLed($drawer) := BLINKING
			if ($drawer=drawer1) then
				outMess($drawer) := TAKE_TYLENOL_IN_10_MIN	
			else 
				if ($drawer=drawer2) then
					outMess($drawer) := TAKE_ASPIRINE_IN_10_MIN	
				else
					outMess($drawer) := TAKE_MOMENT_IN_10_MIN	
				endif
			endif			
		endpar
	...
	CTLSPEC ag(((redLed(drawer1) = ON and not opened(drawer1) and openSwitch(drawer1)) or (redLed(drawer1) = ON and systemTime-drawerTimer(drawer1)=1 and not(opened(drawer1)))) implies ax(redLed(drawer1) = BLINKING))
	CTLSPEC ag(((redLed(drawer2) = ON and not opened(drawer2) and openSwitch(drawer2)) or (redLed(drawer2) = ON and systemTime-drawerTimer(drawer2)=1 and not(opened(drawer2)))) implies ax(redLed(drawer2) = BLINKING))
	CTLSPEC ag(((redLed(drawer3) = ON and not opened(drawer3) and openSwitch(drawer3)) or (redLed(drawer3) = ON and systemTime-drawerTimer(drawer3)=1 and not(opened(drawer3)))) implies ax(redLed(drawer3) = BLINKING))
	// The red light will change value after 10 minutes if the patient doesn't take the pill
	CTLSPEC ag((redLed(drawer1) = ON and systemTime-drawerTimer(drawer1)=1 and not(opened(drawer1))) implies ax(redLed(drawer1) = BLINKING))
	CTLSPEC ag((redLed(drawer2) = ON and systemTime-drawerTimer(drawer2)=1 and not(opened(drawer2))) implies ax(redLed(drawer2) = BLINKING))
	CTLSPEC ag((redLed(drawer3) = ON and systemTime-drawerTimer(drawer3)=1 and not(opened(drawer3))) implies ax(redLed(drawer3) = BLINKING))
	CTLSPEC ag((redLed(drawer1) = BLINKING and systemTime-drawerTimer(drawer1)>1 and not(openSwitch(drawer1))) implies ax(redLed(drawer1) = OFF))
	CTLSPEC ag((redLed(drawer2) = BLINKING and systemTime-drawerTimer(drawer2)>1 and not(openSwitch(drawer2))) implies ax(redLed(drawer2) = OFF))
	CTLSPEC ag((redLed(drawer3) = BLINKING and systemTime-drawerTimer(drawer3)>1 and not(openSwitch(drawer3))) implies ax(redLed(drawer3) = OFF))
	// If the patient takes the pill, red light will turn-off
	CTLSPEC ag((time_consumption(drawer1)<systemTime and not requestSatisfied(drawer1) and opened(drawer1) and not(openSwitch(drawer1)) and not(redLed(drawer1) = OFF) and not(systemTime-drawerTimer(drawer1)>=1)) implies ax(redLed(drawer1) = OFF))
	CTLSPEC ag((time_consumption(drawer2)<systemTime and not requestSatisfied(drawer2) and opened(drawer2) and not(openSwitch(drawer2)) and not(redLed(drawer2) = OFF) and not(systemTime-drawerTimer(drawer2)>=1)) implies ax(redLed(drawer2) = OFF))
	CTLSPEC ag((time_consumption(drawer3)<systemTime and not requestSatisfied(drawer3) and opened(drawer3) and not(openSwitch(drawer3)) and not(redLed(drawer3) = OFF) and not(systemTime-drawerTimer(drawer3)>=1)) implies ax(redLed(drawer3) = OFF))
	...
	main rule r_Main =
		par
			// Set the SystemTime
			systemTime := (systemTime + 1) mod 10
			
			forall $drawer in Drawer do
				if (not(requestSatisfied($drawer)) and systemTime < 10) then
					par					
						...
						if redLed($drawer) = OFF then if (takeThePill($drawer) and not requestSatisfied($drawer)) then r_pillToBeTaken[$drawer] endif endif
						// It is open, drug to be taken, it becomes closed
						if redLed($drawer) = ON and not timeDiffOver600($drawer) and opened($drawer) and not openSwitch($drawer) then r_pillTaken_drawerOpened[$drawer] endif
						...
						if redLed($drawer) = ON then 
							if timeDiffOver600($drawer) then 
